...
typedef NS_ENUM(UInt8, PTZplOrientation) {
    PTZplOrientation_N = 'N',   //normal
    PTZplOrientation_R = 'R',   //rotate 90 degrees (clockwise)
    PTZplOrientation_I = 'I',   // inverted 180 degrees
    PTZplOrientation_B = 'B'   //read from bottom up, 270 degrees
};
...
typedef NS_ENUM(UInt8, PTZplCodeBlockBarCodeMode) {
    // use Code 39 character set
    PTZplCodeBlockBarCodeMode_A = 'A',
    // use Code 128 character set
    PTZplCodeBlockBarCodeMode_F = 'F',
    // use the Code 128 character set and automatically adds FNC1.
    PTZplCodeBlockBarCodeMode_E = 'E'
};

typedef NS_ENUM(UInt8, PTZplCode128Mode) {
    PTZplCode128Mode_N = 'N',   //no selected mode
    PTZplCode128Mode_U = 'U',   //UCC Case Mode
    PTZplCode128Mode_A = 'A',   //Automatic Mode
    PTZplCode128Mode_D = 'D'    //UCC/EAN Mode
};
...
typedef NS_ENUM(UInt8, PTZplQrReliabilityLevel) {
    PTZplQrReliabilityLevel_H = 'H',    //ultra-high reliability level
    PTZplQrReliabilityLevel_Q = 'Q',    //high reliability level
    PTZplQrReliabilityLevel_M = 'M',    //standard level
    PTZplQrReliabilityLevel_L = 'L'     //high density level
};
...
typedef NS_ENUM(UInt8, PTZplTextPrintDirection) {
    //horizontal printing (left to right)
...
typedef NS_ENUM(UInt8, PTZplPrintOrientation) {
    PTZplPrintOrientation_N = 'N',  //normal
    PTZplPrintOrientation_I = 'I'   //invert
};
 ...
 *  \~english
 *
 *  acts as a switch to turn the code validation function on and off.
 *
 ...
 *
 * @note 使用“\&” 回车换行，使用“\(*)”作为软连字符（单词断开时使用），当连字符在一行的最后，会被打印出来，(*)表示任意的数字或字母
 ...
 *
 * @note use “\&” CRLF, use “\(*)”as soft hyphen (use when words break down), when hyphen is at last of the line, it will be printed out, (*) indicates any number or letter.
 ...
 *  \~chinese
 *
 *  缺少坐标时,会假定位于上一个已设置格式的字段后面.
 *  这一记忆法简化了字段相对于其他字段的位置.
 *  在第一个字段定位后,其他字段将自动跟在其后.
 *
 *  \~english
 *
 *  When a coordinate is missing, the position following the last formatted field is assumed.
 *  This remembering simplifies field positioning with respect to other fields.
 *  Once the first field is positioned, other fields follow automatically.
 *
 */
 ...
 *
 *  @param variableFieldDatavariable field data to be printed.
 *
 *  \~english
 *
 ...
 *
 *  生成注册商标、版权符号和其他符号.
 *  使用该指令后，后面紧跟FD命令,通过域数据中的相应字符(A到E)来生成所需符号.
 *
 *  @param orientation 朝向
 ...
 *  \~english
 *
 *  this command enables you to generate the registered trademark, copyright symbol, and other symbols.
 ...
 *  \~english
 *
 *  command is used at the beginning of a label format to load a stored image of a format and merge it with additional data.
 *  The image is always positioned at FO 0,0.
 *
 *  @param objectLocation location of stored object
 *  @param objectName     object name. 1~8 characters
 ...
 *
 *  启用通讯诊断
 *  @note 启动诊断模式,为打印机接收的所有字符生成ASCII打印结果(使用打印机的当前标签长度和全宽).该打印结果包括ASCII字符、十六进制值和任
 何通信错误。
 *
 *  \~english
 *
